...
bool DX11Renderer::CreateMainWindow(int width, int height)
{
	...
	if (IsEditorMode())
	{
		...
		m_hWnd = CreateWindowExA(exstyle, wc.lpszClassName, "Editor", style, x, y, width, height, NULL, NULL, wc.hInstance, NULL);
	...
	else
	{
		...
		m_hWnd = CreateWindowExW(exstyle, wcex.lpszClassName, m_wndCaption.c_str(), style, x, y, width, height, NULL, NULL, wcex.hInstance, NULL);
		if (m_hWnd) {
